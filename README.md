...
本补丁继续复用47份既有migration，不新增或重跑SQL，不修改飞书源表和237条隔离记录。正式试点门禁不变：仍需5个真实production项目、4位不同真实用户本人签署、黄金链A/E、飞书三类真实回执和一次真实失败恢复。
...
受控试点页新增“正式试点启动包”，一次性导出五项目、四角色真人匹配、黄金链 A/E、飞书三类回执和隔离数据的当前差距、操作路径与不可代办边界。生产治理API最终输出为230条样例建议、7条人工判断、0条正式候选，不能用来冒充五个真实试点项目。本版本不新增数据库对象，继续复用 V6.2 隔离台账与 V6.6 试点门禁；详细说明见 `docs/v664-feishu-quarantine-governance-and-pilot-startup.md`。
...
验收状态、项目、参与人、模块检查、黄金链、飞书回执和事件全部持久化到 Supabase；每次写入要求幂等键与期望版本，状态和追加式事件在同一事务保存。系统不允许管理员或自动任务代签，也不允许把测试账号、test 数据或技术演练标为正式通过。飞书消息/任务确认新增受校验的组织、项目和数据空间范围；无范围的旧通用确认不能成为试点证据。详细契约见 `docs/v660-controlled-pilot-acceptance.md`。
...
V6.1.0 是 V6.1–V6.6 全模块真实化计划的第一个版本；V6.2.0 已完成八类飞书事实同步与对账底座，V6.3.0 完成立项与规划持久化，V6.3.1 完成交付控制真实化，V6.3.2 完成业财、干系人、质量与验收链真实化，V6.3.3 完成执行、监控、风险、问题、变更、行动项和收尾贯通，V6.3.4 完成正式汇报、会议、迁移结果和知识成果持久化，V6.4.0 完成四角色工作台与统一收件箱，V6.5.0 完成跨角色事件、飞书身份、角色 AI 和动态知识闭环，V6.6.0 已建立全模块受控试点验收候选；正式推广仍以真实人员和真实业务试点通过为门禁。
