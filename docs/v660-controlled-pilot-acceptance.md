...
V6.6.5统一文档与生产分类器口径：原始载荷启发式扫描与白名单分类器结果分开记录，权威可执行结果为230条样例建议、7条人工判断、0条正式候选；功能、数据库和正式试点门禁不变。
...
V6.6.5 可以作为全模块技术验收候选发布。只有真实受控试点达到正式条件后才允许发布 V7.0；如果试点发现问题，只发布 V6.6.x 修复版本。原始知识资料和 215 组故意保留的冗余不删除、不改写。
