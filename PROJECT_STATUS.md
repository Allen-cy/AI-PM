...
> 当前发布候选：V6.5.0 | 更新日期：2026-07-15 | 阶段：V6.1–V6.6 全模块真实化进行中

V6.5.0 已完成跨角色追加式业务事件链、正式报告复用、组织/个人飞书身份边界、角色 AI 定时扫描与动态知识 RAG。生产 Supabase 已应用四份 V6.5 增量迁移，新增对象全部启用 RLS 并撤销客户端直接权限；业务事件禁止更新和删除。系统下一步进入 V6.6.0 全模块验收候选：自动化与隔离测试数据可以由系统完成，四位真实人员签字、真实业务项目和三类飞书确认写入必须由受控试点完成，不会伪造为已通过。历史 V3.0 报告保留在下方，仅作产品演进记录。
