...
> 当前发布候选：V6.6.8 | 更新日期：2026-07-16 | 阶段：目标镜像闭环及登录后主页浏览器稳定性完成，等待真实受控试点

V6.6.8修复登录后主页因服务端和浏览器分别计算问候语、日期而产生的React hydration mismatch；首屏改用确定文本，挂载后再按本地时间更新。V6.6.7分类写回后的目标镜像闭环保持不变：个人连接与组织事实源必须指向同一Base、领域表和同步流水表；定向对账仅处理同一记录且不墓碑化其他记录。本补丁不新增migration，继续复用生产库48份migration。线上共享Base健康检查为8/8表可用；个人飞书连接仍为0，系统按设计阻断高风险写回且不回退管理员身份。237条源记录未被自动分类或改写，正式试点仍需真实业务人员完成5项目、4真人、黄金链A/E、三类飞书回执和失败恢复。历史V3.0报告保留在下方，仅作产品演进记录。
