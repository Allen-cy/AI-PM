...
> 当前发布候选：V6.6.4 | 更新日期：2026-07-16 | 阶段：正式试点数据治理工具完成，等待真实受控试点

V6.6.4 新增飞书隔离数据分类治理台和正式试点启动包。组织级 PMO 可查看脱敏后的隔离原因与分类建议、下载中文 CSV，并按“正式/样例/测试/诊断”补齐飞书字段后重新对账。生产只读审计确认当前 237 条唯一隔离记录中 234 条带样例或测试标记，3 条必须人工判断，正式项目候选仍为 0；系统不会把这些记录冒充为五个真实项目。本版本不新增数据库对象，继续复用 47 份既有 migration 与安全门禁。当前正式试点仍需 5 个真实项目、4 位不同真实人员本人签署、黄金链 A/E、三类飞书回执和一次真实失败恢复。历史 V3.0 报告保留在下方，仅作产品演进记录。
