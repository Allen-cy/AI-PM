...
const TYPE_LABEL: Record<string, string> = { risk: "风险", joint_check: "联合检查", operating_calendar: "运行日历", governance_approval: "治理审批", management_signal: "管理信号", ai_recommendation: "AI建议", decision_receipt: "决策回执", feishu_confirmation: "飞书确认", formal_output: "正式成果", action: "行动项", closure_review: "收尾审批", benefit_review: "收益复核", correction: "人工纠偏", report_receipt: "汇报接收", evidence_review: "证据核验", data_quality: "数据纠偏", governance_action: "会后行动", capacity_conflict: "资源冲突", project_dependency: "项目依赖" };
...
export default function CollaborationInboxPage() {
  ...
  return <main style={{ minHeight: "100vh", background: "var(--bg)" }}>
    ...
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: 28 }}>
      <section className="card" style={{ background: "linear-gradient(135deg,rgba(6,182,212,.15),rgba(59,130,246,.08))" }}><h1 style={{ fontSize: "1.45rem" }}>一个入口接收自己必须处理的事</h1><p style={{ color: "var(--text2)", lineHeight: 1.8, marginTop: 8 }}>按当前角色、业务范围和数据空间，聚合风险、联合检查、运行日历、治理审批、管理信号、AI建议、决策回执、飞书确认和正式成果；每项都可回到真实来源处理。除 PM、运营、PMO 和 CEO 外，既有发起人、业务负责人、财务和质量角色仍按授权范围使用同一收件箱。</p></section>
