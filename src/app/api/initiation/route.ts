...
export async function POST(request: Request) {
  ...
  try {
    if (operation === "save_initiation") {
      ...
      return json({ status: "succeeded", request_id: requestId, context: responseContext, source: source(resolved.dataClass, String(result.data?.updated_at ?? "")), data: result.data, generated_at: new Date().toISOString(), warnings: [] }, 200, requestId);
    }
    ...
    if (operation === "create_requirement") {
      ...
      return json({ status: "succeeded", request_id: requestId, context: responseContext, requirement: result.data, data: result.data, source: source(resolved.dataClass), generated_at: new Date().toISOString(), warnings: [] }, 201, requestId);
    }
    if (operation === "update_requirement") {
      ...
      return json({ status: "succeeded", request_id: requestId, context: responseContext, requirement: result.data, data: result.data, source: source(resolved.dataClass), generated_at: new Date().toISOString(), warnings: [] }, 200, requestId);
    }
    if (operation === "delete_requirement") {
      ...
      return json({ status: "succeeded", request_id: requestId, context: responseContext, deleted_id: id, data: { deleted_id: id }, source: source(resolved.dataClass), generated_at: new Date().toISOString(), warnings: [] }, 200, requestId);
    }
