...
async function readConfirm(request: Request): Promise<boolean> {
  try {
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  if (!await readConfirm(request)) return json({ request_id: requestId, status: "failed", warning: "执行飞书写入前必须显式传入 confirm=true。" }, 400, requestId);
  ...
  try {
    ...
    if (!claim.claimed) {
      ...
      return json({ request_id: requestId, status: "duplicate", confirmation: duplicate.status === "succeeded" ? duplicate.confirmation : confirmation }, 200, requestId);
    }
    ...
    return json({ request_id: requestId, status: "succeeded", confirmation: completed.status === "succeeded" ? completed.confirmation : confirmation, resource }, 201, requestId);
  } catch (error) {
    ...
    return json({ request_id: requestId, status: "failed", warning: `飞书写入失败：${code}`, confirmation: failed.status === "succeeded" ? failed.confirmation : confirmation }, 502, requestId);
  }
}
