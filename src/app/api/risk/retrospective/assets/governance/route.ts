...
export async function GET(request: Request): Promise<Response> {
  ...
  const status = assetsResult.status === "failed" || logsResult.status === "failed"
    ? "failed"
    : logsResult.status === "not_configured" || assetsResult.status === "not_configured"
      ? "not_configured"
      : "succeeded";
  const warning = [
  ...
  return jsonResponse({
    ...
    status,
    risk_retrospective_governance: governance,
