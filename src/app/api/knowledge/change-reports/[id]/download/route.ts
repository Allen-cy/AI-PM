...
import { getKnowledgeChangeReportDownload } from "@/features/knowledge/lifecycle-repository";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (process.env.AUTH_REQUIRED === "true" && !user) {
    return jsonResponse({ request_id: requestId, status: "unauthorized", warning: "请先登录后再下载知识变更报告。" }, 401, requestId);
  }
  const { id } = await params;
  const result = await getKnowledgeChangeReportDownload(id);
