...
export default function RetrospectiveKnowledgePage(){
  ...
  const [form,setForm]=useState({objectives:"",outcomes:"",deviations:"",rootCauses:"",keyDecisions:"",actionEffects:"",lessons:"",applicability:"",templateGaps:"",governanceSuggestions:"",evidenceIds:[] as string[],ownerUserId:"",dueAt:""});
  ...
  return <main style={{minHeight:"100vh",background:"var(--bg)"}}>
    ...
    <div style={{maxWidth:1320,margin:"0 auto",padding:28}}>
      ...
      <section className="card" style={{marginBottom:18}}><h2>提交有输入、有证据的复盘</h2><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>
        ...
        <BusinessEntityMultiSelect kind="evidence" value={form.evidenceIds} onChange={evidenceIds=>setForm({...form,evidenceIds})} placeholder="选择已核验的项目证据*"/><div style={{display:"grid",gap:8}}><BusinessEntitySelect kind="person" value={form.ownerUserId} onChange={ownerUserId=>setForm({...form,ownerUserId})} placeholder="选择改进责任人"/><input className="input" type="datetime-local" value={form.dueAt} onChange={e=>setForm({...form,dueAt:e.target.value})}/></div>
