...
export default function DecisionCenterPage() {
  ...
  const [draft, setDraft] = useState({ title: "", question: "", decisionType: "continue", decisionMode: "routine", decisionLevel: "executive", authorityMode: "individual", committeeId: "", structuredInput: '{"business_reason":"","forecast":"","risks":"","conditions":""}', emergencyTrigger: "", responseSlaMinutes: "", reviewAt: "", reviewOwnerRole: "pmo", optionA: "", optionAImpact: "", optionB: "", optionBImpact: "", recommendation: "A", impact: "", deadline: "", executionDueAt: "", acceptanceCriteria: "", evidenceType: "", evidenceId: "", evidenceTitle: "", meetingId: "", snapshotId: "", sourceSignalIds: [] as string[] });
  const [report, setReport] = useState({ type: "weekly", periodStart: "", periodEnd: "", narrative: "", sourceSystem: "", sourceId: "", metrics: "{}", target: "" });
  ...
  async function createDraft() {
    if (!context) return;
    try {
      const structuredInput = JSON.parse(draft.structuredInput) as Record<string, unknown>;
  ...
  async function createReport() {
    if (!context) return;
    try {
      const metrics = JSON.parse(report.metrics) as Record<string, unknown>;
      const response = await fetch("/api/reporting/snapshots", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        ...
        source_snapshot_at: new Date().toISOString(), source_definition: { source_system: report.sourceSystem, source_id: report.sourceId }, metrics, exceptions: [], submitted_to_user_id: report.target || null,
      }) });
      const body = await response.json();
  ...
  async function recordMeeting(meetingId: string) {
    ...
    if (disposition === "no_action") conclusion = { type: "no_action", title, rationale: window.prompt("请说明无需处理的依据") || "", review_at: reviewAt };
      ...
      let structuredInput: Record<string, unknown>; try { structuredInput = JSON.parse(draft.structuredInput) as Record<string, unknown>; } catch { setError("标准决策输入JSON不合法。"); return; }
      conclusion = { type: "decision", title, review_at: reviewAt, decision_brief: { title: draft.title || title, decisionQuestion: draft.question, decisionType: draft.decisionType, decisionMode: "routine", decisionLevel: draft.decisionLevel, authorityMode: draft.authorityMode, committeeId: draft.committeeId || null, structuredInput, reviewPlan: { review_at: reviewAt, owner_role: "pmo" }, options: [{ key: "A", label: draft.optionA, consequences: draft.optionAImpact }, { key: "B", label: draft.optionB, consequences: draft.optionBImpact }], recommendation: draft.recommendation, impactSummary: draft.impact, requestedDecisionAt: draft.deadline, executionDueAt: draft.executionDueAt, acceptanceCriteria: draft.acceptanceCriteria, evidence: [{ source_type: draft.evidenceType, source_id: draft.evidenceId, title: draft.evidenceTitle }], meetingId, reportingSnapshotId: draft.snapshotId || null, sourceSignalIds: draft.sourceSignalIds } };
    } else {
  ...
  async function assignMyMeetingDelegate(meetingId: string) {
    ...
    const proxy = candidates.find(item => item.userId === window.prompt(`请输入代理人用户ID：\n${candidates.map(item => `${item.name}（${ROLE_LABELS[item.businessRole] || item.businessRole}）：${item.userId}`).join("\n")}`));
        ...
        {context && ["pm", "operations", "pmo"].includes(context.businessRole) && (
          <section className="card">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 10 }}>
              ...
              <input className="input" placeholder="来源记录/快照ID" value={report.sourceId} onChange={event => setReport(value => ({ ...value, sourceId: event.target.value }))} />
              <input className="input" placeholder={'指标JSON，例如 {"progress":68}'} value={report.metrics} onChange={event => setReport(value => ({ ...value, metrics: event.target.value }))} />
          ...
          <>
            ...
            <section className="card">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 10 }}>
                ...
                <input className="input" placeholder="标准决策输入 JSON" value={draft.structuredInput} onChange={event => setDraft(value => ({ ...value, structuredInput: event.target.value }))} />
                ...
                <input className="input" placeholder="证据类型" value={draft.evidenceType} onChange={event => setDraft(value => ({ ...value, evidenceType: event.target.value }))} />
                <input className="input" placeholder="证据记录ID" value={draft.evidenceId} onChange={event => setDraft(value => ({ ...value, evidenceId: event.target.value }))} />
                <input className="input" placeholder="证据标题" value={draft.evidenceTitle} onChange={event => setDraft(value => ({ ...value, evidenceTitle: event.target.value }))} />
              ...
              return (
                <article key={brief.id} style={{ border: "1px solid var(--border)", borderRadius: 10, padding: 16, background: "var(--surface2)" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                    ...
                    {context?.businessRole === "pmo" && evidenceRequests.filter(item => ["open", "rejected"].includes(asText(item.status))).map(item => <button key={`evidence-submit-${asText(item.id)}`} className="btn-primary" onClick={() => { const response = window.prompt("请填写补证说明"); const evidenceId = window.prompt("请填写补充证据ID"); if (response && evidenceId) patchBrief(brief.id, { action: "resubmit_evidence", evidence_request_id: item.id, response, evidence: [{ source_type: "business_record", source_id: evidenceId, title: "决策补充证据" }] }).catch(cause => setError(cause.message)); }}>提交补证</button>)}
                    ...
                    {receipts.filter(item => item.status === "acknowledged").flatMap(receipt => workspace.executionActionLinks.filter(link => asText(link.receipt_id) === asText(receipt.id)).map(link => { const action = executionActions.find(item => asText(item.id) === asText(link.action_item_id)); if (!action) return null; if (action.status === "accepted") return <button key={`start-${asText(action.id)}`} className="btn-primary" onClick={() => { const comment = window.prompt("请填写执行计划与启动说明"); if (comment) patchBrief(brief.id, { action: "start_execution", receipt_id: receipt.id, action_item_id: action.id, comment }).catch(cause => setError(cause.message)); }}>开始：{asText(action.title)}</button>; if (action.status === "in_progress") return <button key={`evidence-${asText(action.id)}`} className="btn-primary" onClick={() => { const comment = window.prompt("请填写执行结果"); const evidenceId = window.prompt("请填写可验证的执行证据ID"); if (comment && evidenceId) patchBrief(brief.id, { action: "submit_execution_evidence", receipt_id: receipt.id, action_item_id: action.id, comment, evidence: [{ source_type: "business_record", source_id: evidenceId, title: "决策执行证据" }] }).catch(cause => setError(cause.message)); }}>提交证据：{asText(action.title)}</button>; return null; }))}
                    {context && ["pm", "operations", "business_owner", "finance", "quality"].includes(context.businessRole) && ["executing", "effect_review"].includes(brief.workflowStatus) && executionActions.length > 0 && executionActions.every(item => item.status === "evidence_submitted") && <button className="btn-secondary" onClick={() => { const actual = window.prompt("请填写实际效果"); const evidenceId = window.prompt("请填写效果证据ID"); if (actual && evidenceId) patchBrief(brief.id, { action: "submit_effect_review", expected_effect: brief.impactSummary, actual_effect: actual, effect_outcome: "partially_achieved", evidence: [{ source_type: "business_record", source_id: evidenceId, title: "决策效果证据" }], metrics: Object.fromEntries(brief.reviewMetrics.map(metric => [metric, null])) }).catch(cause => setError(cause.message)); }}>提交效果复核</button>}
                    ...
                    {context && ["pmo", "ceo", "sponsor"].includes(context.businessRole) && brief.workflowStatus === "closed" && <button className="btn-secondary" onClick={() => { const condition = window.prompt(`请选择触发的重开条件：\n${brief.revocationConditions.join("\n")}`); const reason = window.prompt("重新打开原因"); const evidenceId = window.prompt("重新打开证据ID"); if (condition && reason && evidenceId) patchBrief(brief.id, { action: "reopen", triggered_condition: condition, reason, evidence: [{ source_type: "business_record", source_id: evidenceId, title: "决策重开证据" }] }).catch(cause => setError(cause.message)); }}>重新打开</button>}
                  </div>
                </article>
              );
            })}
